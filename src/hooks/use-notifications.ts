...
const getUserById = (id: string) => mockUserData.find((user) => user.id === id);
...
const useNotifications = () => {
  ...
  useEffect(() => {
    ...
    const requester1 = getUserById('hong123'); // UX디자이너_123
    const receiver1 = getUserById('kim1233'); // 김영희
    const requester2 = getUserById('park123'); // 박철수
    const receiver2 = getUserById('hong123'); // UX디자이너_123
